...
| Conversation / Message | 会话所有者、Case、消息、最近窗口、历史摘要、引用 | 会话服务 | SQLite 持久化 |
| Document / Chunk | 文档版本、权限、来源、结构化片段 | 企业文档源 | SQLite + Milvus/内存向量 |
| Pending Action | 操作类型、参数、action_id、会话所有者 | Agent 服务 | 服务端会话状态 |
| Ticket | 客户、订单、Case、状态、幂等键、交接摘要、处理结果 | Helpdesk | SQLite 示例表 + FTS5 检索 + 写工具 |
...
3. 选择会话后，系统聚合客户、关联订单、服务期、套餐权益、最近消息和历史摘要。
...
7. 长会话把较早消息压缩为摘要；当前追问只使用当前 Case 的受控上下文。
8. 系统按当前 Case 权限读取同一客户的历史 Ticket，并推荐相似已解决问题。
9. 需要升级时，Agent 先冻结客户、订单、摘要和最近对话，生成 Pending Action。
10. 客服确认后执行幂等创建，将交接快照和 Ticket 关联回 Customer、Order、Support Case，并写入审计。
...
- 会话记忆按 tenant + user + conversation 读取，Case 还必须通过 assignee 校验。
- 历史工单按 tenant + customer + 当前 Case assignee 读取，不能作为跨客户的自由记忆。
- 所有有副作用动作必须产生审计记录。
...
- 重新打开 Case 能恢复最近对话；长会话摘要和工单交接信息可以追溯到原始消息。
