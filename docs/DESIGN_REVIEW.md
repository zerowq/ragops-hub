...
- Conversation 从“只写消息”升级为 Case 级短期记忆，支持最近消息、追问指代和确定性摘要。
- 被安全策略拦截的消息保留审计但不进入后续摘要和生成上下文。
- 工单创建冻结客户、订单、Case、历史摘要和最近对话，保存人工交接快照。
- 历史工单按当前 Case 权限和 customer 边界读取，使用独立 FTS5 索引推荐相似已解决问题。
...
- 会话记忆仍需增加 TTL、PII 脱敏、用户删除权、客服转派和摘要质量评测。
- 历史工单仍需对接真实 Helpdesk，同步处理结果、字段脱敏、客户删除权和推荐效果评测。
