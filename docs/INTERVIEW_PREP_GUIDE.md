...
- 当前支持 Demo Header 和签名 JWT 两种模式；真实企业环境应进一步接入 OIDC/JWKS。
...
- 中文分词的简化处理：单字 + 中文 bigram，并写入 SQLite FTS5 持久倒排索引；生产可换专业分词器。
...
- 幂等键：租户 + 用户 + 服务端 action_id；数据库唯一约束和原子插入兜底。
...
- 当前 5 题、18 passed 只能证明基线链路正确，不能代表生产效果。
...
16. **向量库和关系库双写失败怎么办？**
   - 当前有 processing/failed/deleting、向量补偿和孤立向量过滤；生产继续增加 Outbox、重试和一致性巡检。

17. **用户确认后请求重放会怎样？**
    - 幂等键保证同一动作返回原工单；Pending Action 还应设计为原子消费，生产可用事务/状态机防并发双消费。

18. **为什么当前身份头不安全？**
   - Demo Header 可伪造；JWT 模式从签名 claims 生成 Principal，正式系统再接 OIDC/JWKS，客户端不能自定义租户和用户。
...
不要写“生产承载”“准确率 100%”或“全面防御 Prompt Injection”。更准确的说法是“5 题离线基线中 Source Recall@K=1.0、MRR=1.0，18 项自动化测试通过”。
